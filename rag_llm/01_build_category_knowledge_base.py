...
def group_by_label(dataset: list[dict]) -> dict[str, list[dict]]:
    ...
    for item in dataset:
        required_keys = ["call_id", "doc_type", "unit", "category", "label_name"]

        for key in required_keys:
            if key not in item:
                raise KeyError(f"classifier_dataset.json 缺少欄位：{key}")
...
def validate_kb_item(kb_item: dict, source_item: dict, num_examples: int, data_sufficiency: str) -> dict:
    """
