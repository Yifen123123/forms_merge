...
def get_complaint_info(group_df: pd.DataFrame):
    complaint_risk = False
    complaint_note = ""

    if "complaint_risk" in group_df.columns:
        complaint_risk = (
            group_df["complaint_risk"]
            .apply(clean_bool)
            .any()
        )

    if "complaint_note" in group_df.columns:
        notes = (
            group_df["complaint_note"]
            .dropna()
            .astype(str)
            .str.strip()
        )
        ...
        if not notes.empty:
            complaint_note = notes.iloc[0]

    return complaint_risk, complaint_note


def render_call_block(call_id: str, group_df: pd.DataFrame):
    original_form = load_original_form(call_id)

    complaint_risk, complaint_note = get_complaint_info(group_df)
    ...
    with st.container(border=True):
        ...
        if complaint_risk:
            st.error(
                "🚨 客訴 / 敏感詞風險\n\n"
                f"{complaint_note if complaint_note else '通話內容疑似涉及客訴、申訴或敏感詞。'}"
            )
...
def main():
    ...
    st.caption(
        ...
        "若通話具有客訴或敏感詞風險，會在該 Call ID 區塊上方以紅色標記。"
    )
